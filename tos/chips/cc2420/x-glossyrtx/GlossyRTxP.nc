...
module GlossyRTxP {
  ...
  norace message_t* ONE_NOK m_c_rx_buf;
  uint8_t cur_rx_buf;
  message_t m_rx_buf[RX_BUFFER_SIZE];
  ...
  norace uint16_t cache_dsn;
  norace uint16_t cache_src;
  ...
  #ifdef ENABLE_PR
    uint8_t idx;
    uint8_t* p_header = get_packet_header( m_c_rx_buf );
  ...
  command error_t Init.init() {
    ...
    m_p_tx_buf = &m_tx_buf;
    m_p_rx_buf = &m_rx_buf[cur_rx_buf];
    m_c_rx_buf = m_p_rx_buf;
  ...
  command error_t StdControl.start() {
    ...
    uint16_t setting =  ( 1 << CC2420_MDMCTRL0_RESERVED_FRAME_MODE ) |
  ...
  TOSH_SIGNAL(TIMERB1_VECTOR) {
    ...
    if ( m_ci_state == S_GLOSSY_RECEIVING && ( !(P4IN & (1 << 1)) ) ) {
      ...
      cc2420_header_t* m_ptr_header = (cc2420_header_t*)((uint8_t*)m_p_rx_buf->data - sizeof(cc2420_header_t));
      ...
      if (T_irq <= 8 && quick_forward) {
        ...
        if( (m_ptr_header->dsn == cache_dsn) && (m_ptr_header->src == cache_src) ) {
          quick_forward = FALSE;
        } else {
          radio_start_tx();
        }
    ...
    } else if ( (m_ci_state == S_GLOSSY_WAITING) && (m_tx_state == S_TX_STARTED || m_tx_state == S_TX_ACK_WAITING) && ( (P4IN & (1 << 1)) ) ) {
    ...
    /**According to the Glossy, Rx timeout : packet duration + 200 us
      ...
      if (((cc2420_header_t*)(get_packet_header(m_p_tx_buf)))->fcf & (1 << IEEE154_FCF_ACK_REQ)) {
        atomic m_tx_state = S_TX_ACK_WAITING;
  ...
  static inline void glossy_begin_rx() {
    ...
    // print_var = p_header[0];
    // post print_task();

    if (p_header[0] != packet_len) {
      if (p_header[0] == ACK_LENGTH && m_tx_state == S_TX_ACK_WAITING) {
  ...
  static inline void glossy_end_rx() {
    ...
    if (p_buf[packet_len] >> 7) {
      ...
      cache_dsn = m_ptr_header->dsn;
      cache_src = m_ptr_header->src;
    ...
    m_p_rx_buf = &m_rx_buf[(++cur_rx_buf) % RX_BUFFER_SIZE];
  }
  ...
  static inline void glossy_ack_rx() {
    ...
    fast_read_any( ((uint8_t*)m_rx_header)+1, ACK_LENGTH );
  ...
  static inline void set_mode_spi(msp430_spi_union_config_t* config) {
    atomic {
      ...
      ME1 &= ~(UTXE0 | URXE0);           // disable USART Module
      P3SEL &= ~(0x01 << 4);             // UTXD
      P3SEL &= ~(0x01 << 5);             // URXD
  ...
  task void signal_receive() {
    // post print_packet();
    atomic {
      signal Receive.receive( m_c_rx_buf, m_c_rx_buf->data, packet_len - CC2420_SIZE );
    }
  }
